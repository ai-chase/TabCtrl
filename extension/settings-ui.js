/**
 * settings-ui.js — Settings panel UI for TabCtrl
 *
 * Renders the right-side slide-in settings panel and handles all user
 * interactions (mode toggle, custom rules add/remove/edit, sub-grouping
 * toggle, reset). Reads/writes via window.settingsAPI; re-renders the
 * dashboard via window.renderDashboard on every change.
 */
...
(function () {
  ...
  // Cache flag — settings content is rendered once on init and reused
  // for subsequent opens. Cleared when chrome.storage.local changes.
  let rendered = false;
  ...
  async function openSettings() {
    ensureRefs();
    if (!panelEl) return;
    // Show panel immediately using pre-rendered content (no await render())
    ...
    // First-time fallback: if init() hasn't pre-rendered yet (e.g. settings
    // opened via a deep link before DOMContentLoaded), render now.
    if (!rendered) {
      await render();
      rendered = true;
    }
  ...
  function ensureRefs() {
    if (!panelEl)    panelEl    = document.getElementById('settingsPanel');
    if (!overlayEl)  overlayEl  = document.getElementById('settingsOverlay');
  ...
  function init() {
    ...
    // Pre-render the settings panel content in the background so opening
    // is instant. Render again whenever chrome.storage.local changes
    // (e.g. another tab updates settings).
    ensureRefs();
    if (panelEl) {
      render().then(() => { rendered = true; }).catch(() => {});
    }
    if (chrome.storage && chrome.storage.onChanged) {
      chrome.storage.onChanged.addListener((changes, namespace) => {
        if (namespace !== 'local') return;
        // Any settings-shaped key change → re-render on next open
